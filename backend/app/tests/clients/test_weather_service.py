...
from models.city_data_dto import CityDataDTO, CurrentData, Location, Pollution, Weather
...
def run_get_city_weather_data_test(provider: WeatherService, city: str, country: str, supported_cities: List[str]):
    ...
    mocked_states_response = WeatherApiResponse[StateDTO](
        status='success', 
        data=[
            StateDTO(state="Silesia"),
            StateDTO(state="Greater Poland"),
            StateDTO(state="Kujawsko-Pomorskie"),
        ]
    )
...
def test_get_city_weather_data(weather_service: WeatherService):
    ...
    mocked_city_data = WeatherApiResponse[CityDataDTO](
        ...
        CityDataDTO(
            ...
            location=Location(
                type='Point',
                coordinates=[18.516138055555555, 50.111179444444446]
            ),
            current=CurrentData(
                pollution=Pollution(
