...
def test_get_token_from_header(get_auth_service: AuthService, header_request):
    assert get_auth_service.get_token_from_header(header_request) == 'XYZ'
...
def test_lack_of_token(get_auth_service: AuthService, no_header_request):
    with pytest.raises(HTTPException) as no_token_exception:
        ...
        assert no_token_exception.value.status_code == status.HTTP_401_UNAUTHORIZED
        assert no_token_exception.value.detail == 'Authorization header missing'
...
def test_wrong_token_type(get_auth_service: AuthService, wrong_type_request):
    with pytest.raises(HTTPException) as no_token_exception:
        ...
        assert no_token_exception.value.status_code == status.HTTP_401_UNAUTHORIZED
        assert no_token_exception.value.detail == 'Invalid token type'
...
def test_exception_handler(get_auth_service: AuthService, exception, expected_status, expected_message):
    ...
    assert expected_exception.value.detail == expected_message
    assert expected_exception.value.status_code == expected_status