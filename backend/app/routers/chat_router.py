...
from containers import user_service_dependency, bot_service_dependency, chat_service_dependency, chat_history_dependency, authorize_no_role

router = APIRouter()


@router.get('/chat', response_model=ChatDto)
def get_new_chat(chat_service: chat_service_dependency, user_service: user_service_dependency, decoded_token = Depends(authorize_no_role)) -> Chat:
...
@router.get('/chat/history', dependencies=[Depends(authorize_no_role)])
...
@router.get('/chat/{chat_id}', dependencies=[Depends(authorize_no_role)], response_model=ChatDto)
def get_chat_by_id(chat_id: int, chat_service: chat_service_dependency) -> Chat | None:
    return chat_service.get_chat_by_id(chat_id)

@router.post('/chat', dependencies=[Depends(authorize_no_role)])
